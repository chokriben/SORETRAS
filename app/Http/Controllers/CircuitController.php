...
class CircuitController extends Controller
{
    ...
    public function index(Request $request, $perPage = 10)
    {
        ...
        $Circuits = Circuit::paginate($perPage);
        $Circuits->each(
        ...
        if ($request->page) {
            $Circuits = new LengthAwarePaginator($Circuits, count($Circuits), $perPage, $request->page);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            foreach ($langages as $language) {
                $Circuit->translateOrNew($language)->name = $request->input('name_' . $language);
                $Circuit->translateOrNew($language)->description = $request->input('description_' . $language);
            }

            
            $Circuit->ligne_id = $request->input('ligne_id');
            $Circuit->save();
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $Circuit->ligne_id = $request->input('ligne_id');
            $Circuit->active = $request->input('active');
            $Circuit->save();
            ...
            if ($request->hasFile('file')) {
                ...
                if ($checkPhoto) {
                    $filename = $file->store('Circuit/photos', 'ftp');
                    $media = new Media(
                        [
                            'legende' => 'legende',
                            'type' => '1',
    ...
    {
        Circuit::findOrFail($request->id)->delete();
