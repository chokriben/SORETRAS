...
class CategorieController extends Controller
{
    ...
    public function index(Request $request, $perPage = 10)
    {
        ...
        $Categories->each(
            function ($Categorie, $key) {
                $user = $Categorie->user;
                $ville = $Categorie->ville;
            }
        );

        if ($request->page) {
            $Categories = new LengthAwarePaginator($Categories, count($Categories), $perPage, $request->page);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            foreach ($langages as $language) {
                $Categorie->translateOrNew($language)->name = $request->input('name_' . $language);
                $Categorie->translateOrNew($language)->description = $request->input('description_' . $language);
            }

            $Categorie->klm_debut = $request->input('klm_debut');
            $Categorie->klm_fin = $request->input('klm_fin');
            $Categorie->ligne_id = $request->input('ligne_id');
            $Categorie->save();
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->hasFile('file')) {
                ...
                if ($checkPhoto) {
                    $filename = $file->store('Categorie/photos', 'ftp');
                    $media = new Media(
                        [
                            'legende' => 'legende',
                            'type' => '1',
