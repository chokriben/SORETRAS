...
foreach ($abonnes as $abonne) {
    ...
    if ($a != $result[0]->position) {
        ...
        $pdf->Image($imagePath, 4, 18, 30, 32); // Parameters: image path, x, y, width, height (0 for auto height)
    ...
    if ($abonne->type_abonne == null) {
        ...
        if ($abonne->type_abonne == null) {
            ...
            $pdf->Write(10, 'إشتراك إقليمي صالح إلى');
        }
        $pdf->setRTL(false);
        ...
        foreach ($stations as $station) {
            ...
            /*  DB::table('abonnes')
                ->where('id', $abonne->id)
                ->update(['etat' => 'Imprimer', 'date_imprim' => now()]);*/
        }
    } else {
        $a = 99999999999999999999;
    }
